---
title: 10.5.0 Release
alias: rl/releases/rl10_10.5.0_release.html
description: Version-specific information for the 10.5.0 Release of RightLink.
---

10.5.0 is a major GA release that provides a revamped [Managed SSH Login](/rl10/reference/rl10_managed_ssh_login.html) feature and enables your boot scripts to perform [efficient package management](/rl10/reference/rl10_script_execution.html) in the style of RightLink 5-6. This release includes:

### Features
1. Updated [Managed SSH Login](/rl10/reference/rl10_managed_ssh_login.html) support for Linux
2. Expose RS_PACKAGES to [RightScripts](/rl10/reference/rl10_script_execution.html#background-script-execution-environment), which contains the union of all "packages" entries listed by any RightScript in the boot sequence. This allows ServerTemplate authors to install all packages during an early-stage boot script and avoid repeated, slow invocations of the package manager

### Fixes
1. Fix corner-case bugs with [Docker](/rl10/reference/rl10_docker_support.html) image tagging (import, multiple tags, etc)
2. Do not exit RightLink if the OS shutdown command failed since the init system would restart RightLink and the instance would go back to the operational state which negates any hard shutdown that could be done from RightScale
3. Remove the creation of the 'rightscale' user and the policy updates of the '/home/rightscale/.ssh/authorized_keys' file

## Quick Reference

| Item       | Description |
| ---------- | ----------- |
| Change Log | [https://rightlink.rightscale.com/rll/10.5.0/CHANGES.md](https://rightlink.rightscale.com/rll/10.5.0/CHANGES.md) |
| Github repo for Base ServerTemplates | [https://github.com/rightscale/rightlink_scripts/tree/10.5.0](https://github.com/rightscale/rightlink_scripts/tree/10.5.0) |
| GitHub repo for optional example RightScripts | [https://github.com/rightscale/rightlink_scripts/tree/10.5.0/rll-examples](https://github.com/rightscale/rightlink_scripts/tree/10.5.0/rll-examples) | 
...
| Linux Enable-running | Script: [https://rightlink.rightscale.com/rll/10.5.0/rightlink.enable.sh](https://rightlink.rightscale.com/rll/10.5.0/rightlink.enable.sh)<br>To run: Import Linux Base ServerTemplate, launch raw instance, copy key from<br><b>Settings > Account Settings > API credentials</b>, SSH into raw instance<br>Run command:<br><code>curl -s https://rightlink.rightscale.com/rll/10.5.0/rightlink.enable.sh &#124; bash -s -- -k &quot;&lt;your-key&gt;&quot; -d &quot;&lt;deployment-name&gt;&quot; -n &quot;&lt;your-server-name&gt;&quot; -t &quot;RightLink 10.5.0 Linux Base&quot; -a &quot;us-3.rightscale.com&quot; -c &quot;&lt;cloud_type&gt;&quot;</code> |
...
| Windows Enable-running | Script: [https://rightlink.rightscale.com/rll/10.5.0/rightlink.enable.ps1](https://rightlink.rightscale.com/rll/10.5.0/rightlink.enable.ps1)<br>To run: Import Windows Base ServerTemplate, launch raw instance, copy key from <br><b>Settings > Account Settings > API credentials</b>, Remote Desktop into raw instance<br>Run command:<br><code>$wc = New-Object System.Net.WebClient</code><br><code>$wc.DownloadFile(&quot;https://rightlink.rightscale.com/rll/10.5.0/rightlink.enable.ps1&quot;, &quot;rightlink.enable.ps1&quot;);</code><br><code>Powershell -ExecutionPolicy Unrestricted -File rightlink.enable.ps1 -k &quot;&lt;your-key&gt;&quot; -d &quot;&lt;deployment-name&gt;&quot; -n &quot;&lt;your-server-name&gt;&quot; -t &quot;RightLink 10.5.0 Windows Base&quot; -a &quot;us-3.rightscale.com&quot; -c &quot;&lt;cloud_type&gt;&quot;</code>
...
* From the marketplace import the [RightLink 10.5.0 Linux Base ServerTemplate](https://www.rightscale.com/library/server_templates/RightLink-10/lineage/53250) published by RightScale Engineering.
...
* Standard MCI tags (see [RightLink 10: Cloud-init](../reference/rl10_install_at_boot.html) for additional tags for Linux):
    ...
    `rs_agent:mime_include_url=https://rightlink.rightscale.com/rll/10.5.0/rightlink.boot.sh`
